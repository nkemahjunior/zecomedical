...
import AppointmentRequestTableRow from "./AppointmentRequestTableRow";
import {
...
export default function AppointmentRequests() {
  ...
  const query = useGetAppointmentRequests(Number(page));
    const dataArr = query.data?.content;
    

  //state for disabling all the rows when a change status is happening in the table
  ...
  function filterData(e: FormEvent<HTMLFormElement>) {
    ...
    if (
      ...
      dataFilter = dataArr
        ?.filter((el) => {
          return el.patient_id.patientID.name
            .replaceAll("_", " ")
            .toLocaleLowerCase()
  ...
  return (
    //w-[300%]
    <div
      className={` border-0 border-yellow-600 border-solid 
    ...
    >
      ...
      <div
        className={` border-0 border-solid border-blue-950 w-full bg-white
      ...
      >
        <div className="w-full flex justify-between items-center">
          <h2 className={`${textStylesH4} flex gap-x-2 items-center `}>
            <span>
              <IoFilterOutline />
            </span>{" "}
            Filters
          ...
          <div
            className="bg-stone-200 flex justify-center items-center p-2 rounded-md 
        ...
        <form className=" space-y-[2rem] xl:space-y-12" onSubmit={filterData}>
          ...
          <div className="w-full flex justify-center">
            <button
              className={` text-white text-center w-[30%] 2xl:w-[20%] h-[3.8rem] md:h-[4.6rem] rounded-lg shadow-lg
                                        transition-all delay-75 duration-75
                                        xl:hover:scale-95 ${
                                          disableRow
                                            ? "pointer-events-none cursor-not-allowed bg-[#4d5d62]"
                                            : "bg-[#00171F]"
                                        }`}
            >
              Apply&nbsp;Filter
      ...
      <div
        className="w-[100%] overflow-x-scroll 2xl:overflow-x-hidden  border-2 border-solid border-stone-100
                mt-[4rem]
      ...
      >
        <div className="w-[300%] xl:w-[200%] 2xl:w-full">
          <table className=" table-auto w-full ">
            <caption
              className={`${textStylesH3} text-start pl-4 2xl:pl-0 2xl:text-center  py-4 `}
            >
              Appointment requests
            </caption>
            <thead>
              <tr className="bg-[#e6e8e9] border-b-2 border-solid border-stone-400">
                <th className=" pl-[3rem]  py-[1rem]  lg:py-[1.2rem] text-left pr-[6rem] ">
                  Patient&nbsp;Name
                </th>
                <th className="  py-[1rem]  lg:py-[1.2rem] text-left pr-[6rem]  ">
                  Date
                </th>
                <th className="  py-[1rem]  lg:py-[1.2rem] text-left pr-[6rem]  ">
                  Time
            ...
            <tbody>
              {filteredData?.map((el, i) =>
                //Fragment is thesame thing as <></>

                disableRow ? (
                  <AppointmentRequestTableRow
                    data={el}
                    key={i}
                    current={i}
                    setDisable={setDisableRow}
                    disable={true}
                  />
                ) : (
                  <AppointmentRequestTableRow
      ...
      <div
        className={`${textStylesBody} border-0 border-solid border-black mt-8 xl:mt-16 w-full flex justify-center items-center gap-x-8 sm:gap-x-12`}
      >
        <p>
          showing results {query.data!.number + 1} of {query.data?.totalPages}{" "}
        </p>
      </div>

      <div
        className={`${textStylesBody} border-0 border-solid border-black mt-6 xl:mt-14 w-full flex justify-center items-center gap-x-8 sm:gap-x-12`}
      >
        <button
          onClick={prevPagePagination}
          className="w-[12rem] h-[3rem] sm:w-[15rem] sm:h-[4rem] text-black border-2 border-solid border-[#00171F] rounded-lg bg-stone-50 xl:hover:scale-95 flex justify-center items-center"
        >
          <span>
            <MdOutlineKeyboardDoubleArrowLeft />
          </span>{" "}
