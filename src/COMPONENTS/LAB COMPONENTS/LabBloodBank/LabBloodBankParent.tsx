...
import { useGetLabBloodBankRequests } from "@/DATA_FETCHING/LAB/hooks/useGetLabRequests"
...
import LabBloodBankTableRow from "./LabBloodBankTableRow"
import ButtonSpinner from "@/COMPONENTS/GLOBAL_COMPONENTS/ButtonSpinner"


export default function LabBloodBankParent() {
    ...
    const infiniteQuery = useGetLabBloodBankRequests(searchValue)
                        ...
                        <tbody >

                            {
                                infiniteQuery.data?.pages.map((el) => (
                                    el?.content?.map( (req) => (
                                        <LabBloodBankTableRow data={req} key={req.id}/>
                                    ) )
                                ))
                            }
