...
import LabTableModalDetailsBloodBank from "./LabTableModalDetailsBloodBank"

 

export default function LabBloodBankTableRow({data}:{data:labRequestType}) {

    const {setOpen , setContent} = useContext(LabModalContext) as LabContextTypes

    function openModal(){
        setOpen(true)
        setContent(<LabTableModalDetailsBloodBank el={data}/>)
    }
