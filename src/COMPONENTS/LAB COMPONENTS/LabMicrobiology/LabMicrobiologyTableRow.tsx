...
import LabTableModalDetailsMicrobiology from "./LabTableModalDetailsMicrobiology";


 

export default function LabMicrobiologyTableRow({data}:{data:labRequestType}) {

    const {setOpen , setContent} = useContext(LabModalContext) as LabContextTypes

    function openModal(){
        setOpen(true)
        setContent(<LabTableModalDetailsMicrobiology el={data}/>)
    }
