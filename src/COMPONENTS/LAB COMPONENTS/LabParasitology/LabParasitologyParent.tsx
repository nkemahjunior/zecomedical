...
import { useGetLabParasitologyRequests } from "@/DATA_FETCHING/LAB/hooks/useGetLabRequests"
...
import LabParasitologyTableRow from "./LabImmunologyTableRow"
import ButtonSpinner from "@/COMPONENTS/GLOBAL_COMPONENTS/ButtonSpinner"


 
 
export default function LabParasitologyParent() {
    ...
    const infiniteQuery = useGetLabParasitologyRequests(searchValue)
    ...
    return (
        ...
        >
            ...
            <div className="w-full flex justify-center">
                <h1 className={`${textStylesH3} text-black font-medium `}>Parasitology Laboratory</h1>
            </div>
                        ...
                        <tbody >

                            {
                                infiniteQuery.data?.pages.map((el) => (
                                    el?.content?.map( (req) => (
                                        <LabParasitologyTableRow data={req} key={req.id}/>
                                    ) )
                                ))
                            }
